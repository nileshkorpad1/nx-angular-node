...
Scope relates to a logical grouping, business use-case, or scope. Examples of scope from our sample application are cart-service, booking, shared, and core.
They contain libraries that manage a sub-domain of application logic.
We recommend using folder structure to denote domain. The following folder structure is an example scope hierarchy used to describe the cart-service feature: `shared/cart-service/feature/`.
...
Scopes like "shared" and "cart-service" are example for grouping folders, and "feature" is a library that is nested two levels deep.
This offers a clear indication that this feature belongs to a scope of cart-service which is a sub-scope of shared items. The tag used in this library would be scope:shared, as this is the top-level scope.

```json
shared/                  <---- grouping folder
  data-access/             <---- library

  cart-service/               <---- sub-grouping folder
    data-access/           <---- library
    feature-cart-service/       <---- library
...
The tag for the cart-service feature library as in the previous example would now be `scope:shared, type:feature`.
...
> Example: possible scope and type tags for admin-screens project:
>
> ```json
>  "tags": ["scope:admin-screens", "type:app"]
> ```
...
> Naming: data-access (if nested) or data-access-\* (e.g. data-access-cart-service)
...

{
  "depConstraints": [
    ...
    {
      "sourceTag": "scope:admin-screens",
      "onlyDependOnLibsWithTags": ["scope:admin-screens", "scope:core"]
    }
  ]
}
```
...
- [marketplaceaweb NX architecture tips](https://github.com/marketplaceaweb/marketplacespace/wiki/NX-architecture-tips)
